...
GameHit::GameHit(Animations &ganimators) :
		gsprites(ganimators.gsprites), gdata(
...
int GameHit::hitAnchor(std::string turn) // global turn not used as other value may be considered
	...
	CheckerPos currentCheckerPos = Database::getCheckerPos(idx + 1);
	if (idx == 32) {
...
bool GameHit::fillMoveList(int hm) // called when a checker is grabbed for a move
		{
	gdata.Nmoves = 0;
	CheckerPos currentCheckerPos = Database::getCheckerPos(hm + 1);
...
//        if( hm < 28 )// not in bottom row
				if (hm < 28 && !gdata.jumpMade)    // not in bottom row
					...
					if (hm % 8 == 3 || hm % 8 == 4)    // is edge position
							{
						auto checkCheckerPos4 = Database::getCheckerPos(
								hm + 1 + 4);
						if (checkCheckerPos4.color == "") {
							...
							{
						auto checkCheckerPos3 = Database::getCheckerPos(
								hm + 1 + 3);
						auto checkCheckerPos4 = Database::getCheckerPos(
								hm + 1 + 4);
					...
					} else    // even row ( 0, 2, 4, 6 )
					{
						auto checkCheckerPos4 = Database::getCheckerPos(
								hm + 1 + 4);
						auto checkCheckerPos5 = Database::getCheckerPos(
								hm + 1 + 5);
						if (checkCheckerPos4.color == "") {
							gdata.moveIdxList[gdata.Nmoves++] = hm + 4;
						}
				...
				if (hm < 24)    // check for jumps
					...
					if (hm % 8 == 3)    // on right edge
						...
						auto checkCheckerPos4 = Database::getCheckerPos(
								hm + 1 + 4);
						auto checkCheckerPos7 = Database::getCheckerPos(
								hm + 1 + 7);
						...
						auto checkCheckerPos4 = Database::getCheckerPos(
								hm + 1 + 4);
						auto checkCheckerPos9 = Database::getCheckerPos(
								hm + 1 + 9);
						if (checkCheckerPos4.color == opCol
...
// normal case
	if (idx != -1)        // a valid anchor was hit
		...
		if (gdata.Nmoves > 0)        // move is possible from anchor idx
			...
			try {
				currentCheckerPos = Database::getCheckerPos(idx + 1);
...
void GameHit::gameHitUp() {
	...
	if (hm_f != hm_0) {
		...
		CheckerPos checkerPos = Database::getCheckerPos(hm_f + 1);
		CheckerPos checkerPosFrom = Database::getCheckerPos(hm_0 + 1);
		...
		Database::updateCheckerPos(checkerPosFrom);
		Database::updateCheckerPos(checkerPos);

		// Add record of move
		CheckerRecord newRecord(-1, checkerPosFrom, checkerPos,
				currentState.clock
						+ gdata.gameClock.getElapsedTime().asSeconds());
		Database::insertCheckerRecord(newRecord);
		...
		if (gdata.jumpMade) {
			...
			jumpedCheckerPos = Database::getCheckerPos(jumpIdx + 1);
			...
			Database::updateCheckerPos(jumpedCheckerPos);
