...
struct foreign_key_intermediate_t {
	...
	foreign_key_intermediate_t(C m_) :
			m(m_) {
	}
	...
	template<class O, class F>
	foreign_key_t<C, const F& (O::*)() const> references(
			const F& (O::*getter)() const) {
...
template<class O, class F>
constraints::foreign_key_intermediate_t<const F& (O::*)() const> foreign_key(
		const F& (O::*getter)() const) {
...
template<class O, class F>
constraints::foreign_key_intermediate_t<void (O::*)(F)> foreign_key(
		void (O::*setter)(F)) {
	return {setter};
}
#endif
...
inline std::shared_ptr<sqlite_type> to_sqlite_type(const std::string &str) {
	...
	static std::map<sqlite_type, std::vector<std::regex>> typeMap = { {
			sqlite_type::INTEGER, { std::regex("INT"), std::regex("INT.*"),
//                std::regex("INTEGER"),
					std::regex("TINYINT"), std::regex("SMALLINT"), std::regex(
							"MEDIUMINT"), std::regex("BIGINT"), std::regex(
							"UNSIGNED BIG INT"), std::regex("INT2"), std::regex(
							"INT8"), } }, { sqlite_type::TEXT, { std::regex(
			"CHARACTER\\([[:digit:]]+\\)"), std::regex(
			"VARCHAR\\([[:digit:]]+\\)"), std::regex(
			"VARYING CHARACTER\\([[:digit:]]+\\)"), std::regex(
			"NCHAR\\([[:digit:]]+\\)"), std::regex(
			"NATIVE CHARACTER\\([[:digit:]]+\\)"), std::regex(
			"NVARCHAR\\([[:digit:]]+\\)"), std::regex("CLOB"), std::regex(
			"TEXT"), } }, { sqlite_type::BLOB, { std::regex("BLOB"), } }, {
			sqlite_type::REAL, { std::regex("REAL"), std::regex("DOUBLE"),
					std::regex("DOUBLE PRECISION"), std::regex("FLOAT"),
					std::regex("NUMERIC"), std::regex(
							"DECIMAL\\([[:digit:]]+,[[:digit:]]+\\)"),
					std::regex("BOOLEAN"), std::regex("DATE"), std::regex(
							"DATETIME"), } }, };
	for (auto &p : typeMap) {
		for (auto &r : p.second) {
...
struct default_value_extractor {
	...
	std::shared_ptr<std::string> operator()(
			const constraints::default_t<T> &t) {
		std::stringstream ss;
		/*auto needQuotes = std::is_base_of<text_printer, type_printer<T>>::value;
		 if(needQuotes){
		 ss << "'";
...
struct column_t {
	...
	std::shared_ptr<std::string> default_value() {
		...
		tuple_helper::iterator<std::tuple_size<constraints_type>::value - 1,
				Op...>()(constraints, [&res](auto &v) {
 ...
 */
template<class O, class T, class ...Op>
internal::column_t<O, T, Op...> make_column(const std::string &name, T O::*m,
		Op ...constraints) {
	return {name, m, nullptr, nullptr, std::make_tuple(constraints...)};
}

template<class O, class T, class ...Op>
internal::column_t<O, T, Op...> make_column(const std::string &name,
...
template<class O, class T, class ...Op>
internal::column_t<O, T, Op...> make_column(const std::string &name,
		const T& (O::*getter)() const, void (O::*setter)(T),
		Op ...constraints) {
	return {name, nullptr, getter, setter, std::make_tuple(constraints...)};
}
...
struct limit_t {
	...
	limit_t(decltype(lim) lim_, decltype(has_offset) has_offset_,
			decltype(offset_is_implicit) offset_is_implicit_,
			decltype(off) off_) :
			lim(lim_), has_offset(has_offset_), offset_is_implicit(
					offset_is_implicit_), off(off_) {
	}
...
struct join_iterator<conditions::cross_join_t<T>, Tail...> : public join_iterator<
		Tail...> {
...
struct join_iterator<conditions::left_join_t<T, O>, Tail...> : public join_iterator<
		Tail...> {
...
template<class T, class O, class ...Tail>
struct join_iterator<conditions::join_t<T, O>, Tail...> : public join_iterator<
		Tail...> {
...
template<class T, class O, class ...Tail>
struct join_iterator<conditions::left_outer_join_t<T, O>, Tail...> : public join_iterator<
		Tail...> {
...
template<class T, class O, class ...Tail>
struct join_iterator<conditions::inner_join_t<T, O>, Tail...> : public join_iterator<
		Tail...> {
...
template<class L, class R, typename = typename std::enable_if<
...
template<class T, class ...Args>
core_functions::datetime_t<T, Args...> datetime(T timestring,
		Args ...modifiers) {
	return {timestring, std::make_tuple(modifiers...)};
}
...
struct set_t<L, R, Args...> : public set_t<Args...> {
	...
	set_t(L l_, R r_, Args ...args) :
			Super(std::forward<Args>(args)...), l(std::move(l_)), r(
					std::move(r_)) {
	}
...
//            this->apply_to_col_if(l, internal::is_column<column_type>{});
	...
	void for_each_column_with_field_type(L l) {
		this->apply_to_col_if(l,
				std::is_same<F, typename column_type::field_type> { });
	...
	void for_each_column_exept(L l) {
		...
		this->apply_to_col_if(l,
				std::integral_constant<bool, !has_opt::value> { });
	...
	void for_each_column_with(L l) {
		this->apply_to_col_if(l,
				tuple_helper::tuple_contains_type<Op,
						typename column_type::constraints_type> { });
...
struct table_t {
	...
	table_t(decltype(name) name_, decltype(impl) impl_) :
			name(std::move(name_)), impl(std::move(impl_)) {
	}
	...
	std::vector<std::string> primary_key_column_names() {
		std::vector<std::string> res;
		this->impl.template for_each_column_with<constraints::primary_key_t<>>(
				[&res](auto &c) {
					res.push_back(c.name);
				});
		if (!res.size()) {
	...
	std::vector<std::string> composite_key_columns_names(
			constraints::primary_key_t<Args...> pk) {
		std::vector<std::string> res;
		typedef decltype(pk.columns) pk_columns_tuple;
		res.reserve(std::tuple_size<pk_columns_tuple>::value);
		tuple_helper::iterator<std::tuple_size<pk_columns_tuple>::value - 1,
				Args...>()(pk.columns, [this, &res](auto &v) {
...
struct statement_finalizer {
	...
	statement_finalizer(decltype(stmt) stmt_) :
			stmt(stmt_) {
	}
...
//            return (const char*)sqlite3_column_text(stmt, columnIndex);
		auto bytes = static_cast<const char *>(sqlite3_column_blob(stmt,
				columnIndex));
	...
	std::shared_ptr<T> extract(sqlite3_stmt *stmt, int columnIndex) {
		...
		if (type != SQLITE_NULL) {
			return std::make_shared<T>(
					row_extrator<T>().extract(stmt, columnIndex));
		} else {
			return {};
		}
	}
...
struct storage_impl {
	...
	std::string dump(const O &, sqlite3 *, std::nullptr_t) {
		throw std::runtime_error(
				"type " + std::string(typeid(O).name())
						+ " is not mapped to storage in max");
	}

	bool table_exists(const std::string &tableName, sqlite3 *db) {
		bool res = false;
		std::stringstream ss;
		ss << "SELECT COUNT(*) FROM sqlite_master WHERE type = '" << "table"
				<< "' AND name = '" << tableName << "'";
		auto query = ss.str();
		auto rc =
	...
	void begin_transaction(sqlite3 *db) {
		...
		auto query = ss.str();
		sqlite3_stmt *stmt;
		if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt,
				nullptr) == SQLITE_OK) {
	...
	std::string current_timestamp(sqlite3 *db) {
		...
		auto query = ss.str();
		auto rc = sqlite3_exec(db, query.c_str(),
...
struct storage_impl<H, Ts...> : public storage_impl<Ts...> {
	...
	template<class O, class HH = typename H::object_type>
	std::string dump(const O &o, sqlite3 *,
			typename std::enable_if<std::is_same<O, HH>::value>::type * =
					nullptr) {
		std::stringstream ss;
		ss << "{ ";
...
//            auto index = 0;
	...
	void add_column(table_info &ti, sqlite3 *db) {
		...
		auto query = ss.str();
		sqlite3_stmt *stmt;
		auto prepareResult = sqlite3_prepare_v2(db, query.c_str(), -1, &stmt,
				nullptr);
	...
	sync_schema_result schema_status(sqlite3 *db, bool preserve) {
		...
		if (!gottaCreateTable) {
			...
			if (get_remove_add_columns(columnsToAdd, storageTableInfo,
					dbTableInfo)) {
				...
				if (columnsToAdd.size()) {
					...
					if (!gottaCreateTable) {
						...
						if (res == decltype(res)::old_columns_removed)
							res =
									decltype(res)::new_columns_added_and_old_columns_removed;
						else
	...
	static bool get_remove_add_columns(std::vector<table_info*>& columnsToAdd,
		...
		for (size_t storageColumnInfoIndex = 0;
			...
			auto dbColumnInfoIt = std::find_if(dbTableInfo.begin(),
					dbTableInfo.end(), [&](auto &ti) {
						return ti.name == columnName;
					});
			if (dbColumnInfoIt != dbTableInfo.end()) {
				auto &dbColumnInfo = *dbColumnInfoIt;
				auto dbColumnInfoType = to_sqlite_type(dbColumnInfo.type);
				auto storageColumnInfoType = to_sqlite_type(
						storageColumnInfo.type);
				if (dbColumnInfoType && storageColumnInfoType) {
					auto columnsAreEqual =
							dbColumnInfo.name == storageColumnInfo.name
									&& *dbColumnInfoType
											== *storageColumnInfoType
									&& dbColumnInfo.notnull
											== storageColumnInfo.notnull
									&& bool(dbColumnInfo.dflt_value.length())
											== bool(
													storageColumnInfo.dflt_value.length())
					...
					storageTableInfo.erase(
							storageTableInfo.begin() + storageColumnInfoIndex);
					--storageColumnInfoIndex;
				} else {
...
struct storage_t {
	...
	struct view_t {
		...
		view_t(storage_t &stor, decltype(connection) conn, Args ...args) :
				storage(stor), connection(conn), query([&] {
					std::string q;
		...
		public:
			iterator_t(sqlite3_stmt * stmt_, view_t<T, Args...> &view_) :
		...
		iterator_t begin() {
			...
			auto ret = sqlite3_prepare_v2(db, this->query.c_str(), -1, &stmt,
					nullptr);
			if (ret == SQLITE_OK) {
				return {stmt, *this};
			} else {
	...
	storage_t(const std::string &filename_, impl_type impl_) :
			filename(filename_), impl(impl_), inMemory(
					filename_.empty() or filename_ == ":memory:") {
		if (inMemory) {
			this->currentTransaction = std::make_shared<
					internal::database_connection>(this->filename);
...
protected:
	...
	std::string serialize_column_schema(constraints::primary_key_t<Cs...> fk) {
		...
		tuple_helper::iterator<std::tuple_size<decltype(fk.columns)>::value - 1,
				Cs...>()(fk.columns, [&](auto &c) {
	...
	std::string string_from_expression(T t, bool /*noTableName*/= false,
		...
		if (isNullable and !type_is_nullable<T>()(t)) {
			...
			auto needQuotes =
					std::is_base_of<text_printer, type_printer<T>>::value;
			std::stringstream ss;
			if (needQuotes) {
				ss << "'";
			}
	...
	std::string string_from_expression(const char *t, bool /*noTableName*/=
	...
	template<class F, class O>
	std::string string_from_expression(const F* (O::*g)() const,
			bool noTableName = false, bool /*escape*/= false) {
		std::stringstream ss;
		if (!noTableName) {
			ss << " '" << this->impl.template find_table_name<O>() << "'.";
	...
	template<class F, class O>
	std::string string_from_expression(void (O::*s)(F),
			bool noTableName = false, bool /*escape*/= false) {
		std::stringstream ss;
		if (!noTableName) {
			ss << " '" << this->impl.template find_table_name<O>() << "'.";
	...
	std::string string_from_expression(
			aggregate_functions::group_concat_double_t<T> &f,
			bool /*noTableName*/= false, bool /*escape*/= false) {
		std::stringstream ss;
	...
	template<class T>
	std::string string_from_expression(
			aggregate_functions::group_concat_single_t<T> &f,
			bool /*noTableName*/= false, bool /*escape*/= false) {
		std::stringstream ss;
		auto expr = this->string_from_expression(f.t);
	...
	template<class X, class Y>
	std::string string_from_expression(core_functions::ltrim_double_t<X, Y> &f,
			bool /*noTableName*/= false, bool /*escape*/= false) {
		std::stringstream ss;
	...
	template<class X>
	std::string string_from_expression(core_functions::ltrim_single_t<X> &f,
			bool /*noTableName*/= false, bool /*escape*/= false) {
		std::stringstream ss;
		auto expr = this->string_from_expression(f.x);
		ss << static_cast<std::string>(f) << "(" << expr << ") ";
	...
	template<class X, class Y>
	std::string string_from_expression(core_functions::trim_double_t<X, Y> &f,
			bool /*noTableName*/= false, bool /*escape*/= false) {
		std::stringstream ss;
		auto expr = this->string_from_expression(f.x);
		auto expr2 = this->string_from_expression(f.y);
	...
	template<class X>
	std::string string_from_expression(core_functions::trim_single_t<X> &f,
			bool /*noTableName*/= false, bool /*escape*/= false) {
		std::stringstream ss;
		auto expr = this->string_from_expression(f.x);
		ss << static_cast<std::string>(f) << "(" << expr << ") ";
	...
	template<class T, class ...Args>
	std::string string_from_expression(
			core_functions::datetime_t<T, Args...> &f, bool /*noTableName*/=
					false, bool /*escape*/= false) {
		std::stringstream ss;
		ss << static_cast<std::string>(f) << "("
				<< this->string_from_expression(f.timestring);
		typedef std::tuple<Args...> tuple_t;
		tuple_helper::iterator<std::tuple_size<tuple_t>::value - 1, Args...>()(
				f.modifiers, [&](auto &v) {
	...
	template<class T, class ...Args>
	std::string string_from_expression(core_functions::date_t<T, Args...> &f,
			bool /*noTableName*/= false, bool /*escape*/= false) {
		std::stringstream ss;
		ss << static_cast<std::string>(f) << "("
				<< this->string_from_expression(f.timestring);
	...
	std::string string_from_expression(core_functions::random_t &f,
			bool /*noTableName*/= false, bool /*escape*/= false) {
		std::stringstream ss;
		ss << static_cast<std::string>(f) << "() ";
		return ss.str();
	...
	std::string process_where(conditions::is_null_t<T> &c) {
		std::stringstream ss;
		ss << this->string_from_expression(c.t) << " "
	...
	std::string process_where(conditions::and_condition_t<L, R> &c) {
		std::stringstream ss;
		ss << " (" << this->process_where(c.l) << ") "
				<< static_cast<std::string>(c) << " ("
				<< this->process_where(c.r) << ") ";
		return ss.str();
	}
	...
	std::string process_where(conditions::like_t<A, T> &l) {
		std::stringstream ss;
		ss << this->string_from_expression(l.a) << " "
				<< static_cast<std::string>(l) << " "
				<< this->string_from_expression(l.t) << " ";
		return ss.str();
	}
	...
	std::string process_where(conditions::between_t<A, T> &bw) {
		...
		ss << expr << " " << static_cast<std::string>(bw) << " "
				<< this->string_from_expression(bw.b1) << " AND "
				<< this->string_from_expression(bw.b2) << " ";
		return ss.str();
	}
	...
	template<class T, class O>
	void process_single_condition(std::stringstream &ss,
			conditions::left_outer_join_t<T, O> l) {
		ss << static_cast<std::string>(l) << " ";
		ss << " '" << this->impl.template find_table_name<T>() << "' ";
		this->process_join_constraint(ss, l.constraint);
	}
	...
	template<class O>
	void process_single_condition(std::stringstream &ss,
			conditions::order_by_t<O> orderBy) {
	...
	template<class ...Args>
	void process_single_condition(std::stringstream &ss,
			conditions::group_by_t<Args...> groupBy) {
		std::vector<std::string> expressions;
		typedef std::tuple<Args...> typle_t;
		tuple_helper::iterator<std::tuple_size<typle_t>::value - 1, Args...>()(
				groupBy.args, [&](auto &v) {
					auto expression = this->string_from_expression(v);
...
public:
	...
	view_t<T, Args...> iterate(Args ...args) {
		...
		if (!this->currentTransaction) {
			connection = std::make_shared<internal::database_connection>(
					this->filename);
	...
	void remove(I id) {
		...
		if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt,
			...
			statement_finalizer finalizer { stmt };
			auto index = 1;
			impl.table.template for_each_column_with<
					constraints::primary_key_t<>>([&] (auto c) {
				typedef typename decltype(c)::field_type field_type;
	...
	void update(const O &o) {
		...
		ss << "WHERE ";
		auto primaryKeyColumnNames = impl.table.template column_names_with<
	...
	void update_all(internal::set_t<Args...> set, Wargs ...wh) {
		...
		if (tableNamesSet.size()) {
			if (tableNamesSet.size() == 1) {
				...
				set.for_each(
						[this, &setPairs](auto &lhs, auto &rhs) {
				...
				throw std::runtime_error(
						"too many tables specified - UPDATE can be performed only for a single table");
			}
		} else {
...
protected:
	...
	template<class T>
	std::set<std::string> parse_table_name(
			aggregate_functions::group_concat_double_t<T> &f) {
	...
	template<class T, class ...Args>
	std::set<std::string> parse_table_name(
			core_functions::date_t<T, Args...> &f) {
	...
	template<class T, class ...Args>
	std::set<std::string> parse_table_name(
			core_functions::datetime_t<T, Args...> &f) {
		auto res = this->parse_table_name(f.timestring);
		typedef decltype(f.modifiers) tuple_t;
		tuple_helper::iterator<std::tuple_size<tuple_t>::value - 1, Args...>()(
				f.modifiers, [&](auto &v) {
	...
	template<class X>
	std::set<std::string> parse_table_name(
			core_functions::trim_single_t<X> &f) {
		return this->parse_table_name(f.x);
	}

	template<class X, class Y>
	std::set<std::string> parse_table_name(
			core_functions::trim_double_t<X, Y> &f) {
	...
	template<class X>
	std::set<std::string> parse_table_name(
			core_functions::ltrim_single_t<X> &f) {
		return this->parse_table_name(f.x);
	}

	template<class X, class Y>
	std::set<std::string> parse_table_name(
			core_functions::ltrim_double_t<X, Y> &f) {
		auto res = this->parse_table_name(f.x);
		auto res2 = this->parse_table_name(f.y);
		res.insert(res2.begin(), res2.end());
		return res;
	...
	template<class ...Args>
	std::set<std::string> parse_table_names(
			internal::columns_t<Args...> &cols) {
		std::set<std::string> res;
...
//            data_tuple_t data{&res, &impl};
	...
	O get(Ids ...ids) {
		...
		if (primaryKeyColumnNames.size()) {
			...
			if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt,
				...
				constexpr const auto idsCount = std::tuple_size<
						decltype(idsTuple)>::value;
				tuple_helper::iterator<idsCount - 1, Ids...>()(idsTuple,
						[stmt, &index](auto &v) {
			...
			throw std::runtime_error(
					"table " + impl.table.name + " has no primary key column");
		}
	}
	...
	std::shared_ptr<O> get_no_throw(Ids ...ids) {
		...
		auto primaryKeyColumnNames = impl.table.primary_key_column_names();
		if (primaryKeyColumnNames.size()
				and primaryKeyColumnNames.front().length()) {
...
//                            throw not_found_exception{};
	...
	int count(Args ...args) {
		...
		int res = 0;
		std::stringstream ss;
		ss << "SELECT " << static_cast<std::string>(sqlite_orm::count())
				<< "(*) FROM '" << impl.table.name << "' ";
		this->process_conditions(ss, args...);
		auto query = ss.str();
		auto rc = sqlite3_exec(db, query.c_str(),
				[](void *data, int argc, char **argv,char **) -> int {
	...
	double total(F O::*m, Args ...args) {
		...
		if (columnName.length()) {
			...
			if (tableNamesSet.size()) {
				ss << "FROM ";
				std::vector<std::string> tableNames(tableNamesSet.begin(),
						tableNamesSet.end());
	 ...
	 */
	template<class T, class ...Args,
			class R = typename internal::column_result_t<T, Ts...>::type>
	 ...
	 */
	template<class ...Args, class R = std::tuple<
			typename internal::column_result_t<Args, Ts...>::type...>,
			class ...Conds>
...
protected:
	...
	sync_schema_result sync_table(
		...
		ss << "INDEX IF NOT EXISTS " << impl->table.name << " ON '"
				...
				<< "' ( ";
		std::vector<std::string> columnNames;
		tuple_helper::iterator<std::tuple_size<columns_type>::value - 1, Cols...>()(
				impl->table.columns, [&columnNames, this](auto &v) {
		...
		if (schema_stat != decltype(schema_stat)::already_in_sync) {
			if (schema_stat == decltype(schema_stat)::new_table_created) {
				...
				if (schema_stat == sync_schema_result::old_columns_removed
					...
					//  now get current table info from db using `PRAGMA table_info` query..
					auto dbTableInfo = impl->get_table_info(impl->table.name,
							db);
...
//                                       (void)columnName;
	...
	// ADDED CODE by Nick-Yahr 08/27/2017
	/*
	 * Used to get a connection to the current database so that you can
	 * manually do SQLite stuff if need be (such as back up the database).
	 */
	std::shared_ptr<internal::database_connection> getConnection() {
